...
score: 6.0
evidence: 基于开源预训练模型的流式目标说话人识别流水线
tldr: 针对助听器中低至10毫秒可感知延迟的挑战，提出一种基于开源预训练模型的流式目标说话人识别流水线。该方法先进行低延迟流式说话人分割，再对注册目标进行说话人验证。在17个真实播客片段上，系统达到超过0.90的中位准确率与0.95-0.98的特异性，验证了低延迟选择性放大的可行性。
source: arxiv
...
motivation: 助听器需极低延迟识别目标说话人，现有离线方法无法满足实时性要求。
method: 采用Diart流式分割与Pyannote说话人验证，构成低延迟两步流水线。
result: 在17个测试片段上中位准确率超过0.90，特异性达0.95-0.98。
conclusion: 证明了低延迟目标说话人识别用于选择性放大的实际可行性。
---

## 摘要
我们提出了一种由开源预训练模型组成的实时流水线，用于目标说话人的流式识别，其动机源于助听器应用中低至10毫秒的延迟即可被感知。我们采用两步方法：首先使用低延迟流式说话人分割对输入音频进行说话人分段，随后针对已注册的目标说话人进行说话人验证。为在模拟对话语音的同时最小化重叠，我们使用了“This American Life”播客转录数据集，并选择主持人作为一致的目标说话人。我们使用说话人分割错误率（DER）对Pyannote和LIUM的离线分割进行基准测试，并基于基线性能和流式兼容性选择Pyannote。随后，我们使用Pyannote和TitaNet-Large评估说话人验证性能，并生成ROC曲线以选择工作区域。我们集成Diart并调整聚类参数以降低DER，同时保持实时运行。我们将Diart与Pyannote验证配对，通过将预测和真实语音区域转换为100毫秒二进制掩码来评估系统级性能。在17个评估集上，该系统在中位数准确率上超过0.90，在余弦距离阈值0.7-0.75下具有高特异性（0.95-0.98），为下游低延迟选择性放大提供了实用的概念验证。
