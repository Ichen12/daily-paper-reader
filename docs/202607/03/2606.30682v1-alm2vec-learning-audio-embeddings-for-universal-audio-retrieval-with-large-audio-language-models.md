...
score: 6.0
evidence: 通用音频检索嵌入，与语音和音频相关
tldr: 现有音频检索嵌入主要针对音频-字幕匹配优化，难以支持多样化检索目标。ALM2Vec从预训练大型音频语言模型（LALMs）中提取通用嵌入，将音频理解、指令遵循和推理能力迁移到统一嵌入空间。该方法在标准音频和语音检索基准上取得竞争性能，并支持指令感知、组合和可控检索行为。ALM2Vec展示了作为跨领域、任务和用户意图的统一音频嵌入模型的潜力。
source: arxiv
...
motivation: 现有对比双编码器架构的嵌入仅优化音频-字幕匹配，无法灵活支持多样化检索目标和可控行为。
method: 基于预训练大型音频语言模型（LALMs）提取通用音频嵌入，并引入自然语言指令融入嵌入过程实现指令感知检索。
result: 在标准音频和语音检索基准上取得竞争性能，并展现出组合和可控检索能力，如音频问答和方面条件检索。
conclusion: ALM2Vec作为统一音频嵌入模型，有效支持跨领域、任务和用户意图的检索，具有广泛适用性。
---

## 摘要
近期语言-音频检索的进展主要受对比式双编码器架构驱动，该架构在共享嵌入空间中对齐音频和文本。尽管有效，现有检索嵌入主要针对音频-文本匹配优化，限制了其支持多样化检索目标和可控检索行为的能力。我们提出ALM2Vec，一种从预训练的大型音频-语言模型（LALMs）导出的通用音频嵌入框架。通过迁移通过大规模多模态训练获得的音频理解、指令遵循和推理能力，ALM2Vec学习了一个统一的嵌入空间，用于跨音频领域和任务类型的检索。除了传统的文本-音频检索，ALM2Vec将自然语言指令融入嵌入过程，实现了面向音频问答和方面条件检索等场景的指令感知检索。实验结果显示，ALM2Vec在标准音频和语音检索基准上取得了有竞争力的性能，同时展现出有前景的组合和可控检索能力，突显了其作为跨领域、任务和用户意图的统一音频嵌入模型的潜力。
